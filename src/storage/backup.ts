import type { BackupPayload, LightweightExportPayload } from '../domain/types';
...
async function readBackupPayload(): Promise<BackupPayload> {
  ...
  try {
    const payload: BackupPayload = {
      ...
      researchSnapshots: await db.getAll('researchSnapshots'),
...
function isStoredSetting(value: unknown): value is { key: string; value: unknown } {
  return (
    Boolean(value) &&
    typeof value === 'object' &&
...
function assertBackupPayload(payload: BackupPayload): void {
  if (
    ...
    !Array.isArray(payload.researchSnapshots) ||
    (payload.holdingsSnapshots !== undefined && !Array.isArray(payload.holdingsSnapshots)) ||
    !payload.settings.every(isStoredSetting) ||
    !payload.transactions.every(hasId) ||
    !payload.marketCache.every(hasId) ||
    !payload.researchSnapshots.every(hasId) ||
    !(payload.holdingsSnapshots ?? []).every(hasId)
  ) {
...
export async function restoreBackup(payload: BackupPayload): Promise<void> {
  ...
  try {
    const transaction = db.transaction(
      ['settings', 'transactions', 'marketCache', 'researchSnapshots', 'holdingsSnapshots'],
      'readwrite',
    );
    ...
    await Promise.all([
      ...
      transaction.objectStore('researchSnapshots').clear(),
    ...
    await Promise.all([
      ...
      ...payload.marketCache.map((record) =>
        transaction.objectStore('marketCache').put(record),
      ),
      ...payload.researchSnapshots.map((record) =>
        transaction.objectStore('researchSnapshots').put(record),
      ),
